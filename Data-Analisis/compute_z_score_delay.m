...
% case, it's every 50 miliseconds.
...
binWidthPos = .25; 
positions = [-.5 .24 1.25 2.25 3.25];

% Each alignment window width
binWidth = {.25, .25, .25}; 
TimeAxis = ['{ (-0.5+binWidth{j}/2):0.05:(1.5-binWidth{j}/2),' ...
            '  (-1.5+binWidth{j}/2):0.05:(5.5-binWidth{j}/2),' ... % 50 ms sliding window 
...
for i_neuron = 1:length(e_KD) % Loop through each (e)xperimentd, neuron!!
    ...
    for i_trial = 1:length(correct)
        ...
        % Position 2 --> 0.28 s
        p2 = firingrate([correct(i_trial).spikeTimes],positions(2),'filtertype',filterType,'timeconstant',.30);
