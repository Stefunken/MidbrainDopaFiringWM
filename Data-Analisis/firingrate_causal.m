function fRates = firingrate_causal(SpikeTimes, TimeSamples, varargin)
...
for k = 1:ntrials
   ...
   switch lower(FilterType)
      case 'boxcar' % Counts spikes between -TimeConstant/2 and +TimeConstant2 of each time sample (i.e. it extends into the past and future). 
         fRate  = (sum(spkT>=(timesamples-TimeConstant) & spkT<(timesamples),1)/TimeConstant);
         %fRate  = (sum(spkT>=(timesamples) & spkT<(timesamples+TimeConstant),1)/TimeConstant);
